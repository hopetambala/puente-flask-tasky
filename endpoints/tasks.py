...
class Task(Resource):
    def get(self, task_id):
        ...
        print(tasks)
        if tasks:
            return {"task": [task.json() for task in tasks]}, 200
        else:
...
class Tasks(Resource):
    ...
    def post(self):
        ...
        if task:
            ...
            TasksModel.add_task(data_payload["name"], data_payload["task_workflows_id"])
            return (
                {
                    "message": 'Task "{name}" successfully added to database!'.format(
